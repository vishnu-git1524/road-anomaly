...
def load_yolo_model(path: str):
    # MOCK: Always return a dummy model and class names for frontend-only demo
    class DummyModel:
        def predict(self, frame, conf=0.5, verbose=False):
            # Return a dummy result object with empty detections
            class DummyResult:
                pass
            return [DummyResult()]
    dummy_names = {0: "ClassA", 1: "ClassB"}
    logger.info(f"[MOCK] Pretending to load model from {path}")
    return DummyModel(), dummy_names
...
def process_frame(
    ...
    # MOCK: Just draw a rectangle and label for demo
    annotated_frame = frame.copy()
    h, w = annotated_frame.shape[:2]
    cv2.rectangle(annotated_frame, (int(w*0.2), int(h*0.2)), (int(w*0.8), int(h*0.8)), (0, 255, 0), 2)
    cv2.putText(annotated_frame, "[Demo] No real detection", (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
